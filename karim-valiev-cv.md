# Karim Valiev

[valievkarim@gmail.com](mailto:valievkarim@gmail.com)  
**LinkedIn:** [linkedin.com/in/valievkarim](https://linkedin.com/in/valievkarim)  
**GitHub:** [github.com/valievkarim](https://github.com/valievkarim)  
**Telegram:** [@valievkarim](https://t.me/valievkarim)  
**Telegram channel:** [@ciso_on_fire](https://t.me/ciso_on_fire)  
🇬🇧 UK Global Talent visa holder, based in 🇬🇪 Tbilisi, Georgia · Moving to London

---

### Summary

Cybersecurity leader and software engineer building AI-agent products. Deep expertise in Python, Linux, networking and databases. Built an autonomous AI pentesting platform. Established a security program at an internet company with 100+ products. Experienced in hiring and mentoring engineers while staying close to the code.

---

### 💼 Experience

**Co-founder & CTO, Autonomous AI Pentesting Platform** *(2025–present)*

- Built an end-to-end platform turning company scope into verified, client-ready pentest reports
- Implemented large-scale deterministic scanning, parallel AI agent execution and evidence verification; evaluated on XBOW, Vulhub and custom benchmarks
- Built an automated development pipeline: specs → Linear → Codex/Claude → tests → GitHub PRs
- Delivered pilots that uncovered critical vulnerabilities and closed three client sales with my co-founder

**Founder & Developer, Attack Surface Management Tool** *(2022–2025)*

- Built an attack surface management platform with a Python scheduler and custom & OSS modules, continuously monitoring 100k+ hosts
- Optimized scanning throughput and resource utilization with adaptive scheduling and congestion/packet-loss controls
- One-click cloud and bare-metal deployment (Ansible, Docker, GitHub Actions); React dashboard and Telegram alerts; automated tests and performance profiling

**CISO, VK / Mail.Ru** *(2017–2021)*

- Built a company-wide security program for 100+ products; scaled the team 7 → 50, established per-product security units and launched a 24×7 SOC + IR center
- Hardened infrastructure: per-service firewalls, YubiKey-based SSH/VPN, certificate-based Wi-Fi and automated vulnerability scanning
- Built and ran a [top-7](https://www.zdnet.com/article/hackerones-2020-top-10-public-bug-bounty-programs/) global HackerOne bug-bounty program by total payouts (2020) and an ongoing third-party red-team program
- Led incident response and crisis communications; drove M&A security

**Security Team Lead, VK / Mail.Ru** *(2014–2017)*  
**Security Analyst, VK / Mail.Ru** *(2013–2014)*  
**Software Developer, Stream.Ru** *(2012–2013)*  

<div style="break-before: page;"></div>

### 🛠️ Core Skills

- **Dev:** Python, Go, C/C++, LLM agent systems (LangGraph), Linux kernel & eBPF, Docker/K8s, KVM/QEMU, CI/CD (Ansible, GitHub Actions), MySQL, Elasticsearch, AWS & GCP
- **Networking:** TCP/IP (L2–L4), Firewalls, datacenter & office networks, VLANs, Scapy
- **Security:** Pentesting & Bug Bounty, Incident Response / SOC, Vulnerability Management & Code Audits, PKI & Cryptography
- **Mobile / IoT:** Android internals (NDK, Magisk, Frida), OpenWrt porting (previously unsupported router), ESP8266/ESP32, Bluetooth & Zigbee, Home Assistant & ESPHome
- **AI-augmented:** Binary reverse engineering, Linux kernel debugging, frontend development
- **Leadership:** 7 yrs tech management — hiring, mentoring, project & security program delivery

---

### 🌍 Community & Recognition

- **Open-source:**  
  [https://github.com/valievkarim/spinrun](https://github.com/valievkarim/spinrun) - tool to boot a clean, disposable Alpine Linux VM using QEMU in < 0.7s

  Bugfixes to public projects: Ansible *community.general* (2025, included in Ansible), Nuclei scanner (2023), SQLAlchemy (2022), html5lib (2013), MediaInfo (2013)

- **Vulnerability research:**  
 Featured in Halls of Fame from [Google](https://bughunters.google.com/profile/d4114252-2744-4a26-a7d3-0536967a95c3/awards), [Yandex](https://yandex.com/bugbounty/i/hall-of-fame/), [Mozilla](https://www.mozilla.org/en-US/security/bug-bounty/web-hall-of-fame/), Microsoft, [Alibaba](https://security.alibaba.com/top.htm?tab=1) and [Salesforce](https://security.salesforce.com/security-research-contributors) (2015–2021)

- **CTF:**  
  Founded VK team – ranked #7 in Russia (2020–2021)  
  Former core player, MSU "Bushwhackers" – multiple #1 RU/international wins (2010–2015)

- **Public speaking:**  
  - *Launch is Tomorrow* podcast – [modern auth](https://www.youtube.com/watch?v=qWK4ybUfew8) (2024), [security at scale](https://open.spotify.com/episode/32gEhH1lJWZAlA4GSdBw9a) (2019)
  - Conference talks and panels on [applied cryptography](https://www.dropbox.com/s/3a8elbjb6adzr33/%5BDefensive%20Track%5D%20Karim%20Valiev%20-%20How%20to%20Stop%20Using%20Cryptography%20and%20Start%20Living-mKSQ6DjBz3w.mp4?dl=0), [scanner benchmarking](https://confidence-conference.org/archiwum/confidence_10.html) and [cybersecurity careers](https://broadcast.comdi.com/watch/ri9h10rj) (2012–2021)

---

### 🎓 Education

**M.Sc. Mathematics & Computer Science**, Moscow State University *(2008–2013)*
