.PHONY: all html clean open copy pdfv2 clean-all full release
...
clean:
	rm -f output/karim-valiev-cv.html output/karim-valiev-cv-v2.pdf tmp/prince.pdf tmp/NotoColorEmoji.ttx output/res tmp/cleaned_up.pdf
	rm -rf _site
...
full: clean-all venv pdfv2 open
